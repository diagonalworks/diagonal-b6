...
func TestFindReachableFeatures(t *testing.T) {
	...
	collection, err := reachable(&context, origin, b6.ArrayValuesCollection[b6.Tag]([]b6.Tag{{Key: "mode", Value: b6.NewStringExpression("walk")}}).Collection(), 1000.0, b6.Keyed{Key: "#amenity"})
	if err != nil {
...
func TestPathsToReachFeatures(t *testing.T) {
	...
	collection, err := pathsToReachFeatures(&context, origin, b6.ArrayValuesCollection[b6.Tag]([]b6.Tag{{Key: "mode", Value: b6.NewStringExpression("walk")}}).Collection(), 1000.0, b6.Keyed{"#amenity"})
	if err != nil {
		t.Fatalf("Expected no error, found: %s", err)
	}
