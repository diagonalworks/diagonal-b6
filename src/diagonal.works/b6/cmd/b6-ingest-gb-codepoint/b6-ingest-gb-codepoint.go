...
func (p *Postcodes) Read(options ingest.ReadOptions, emit ingest.Emit, ctx context.Context) error {
	...
	feature := ingest.GenericFeature{
		Tags: []b6.Tag{{Key: "#place", Value: b6.NewStringExpression("postal_code")}},
	}
	for i := range p.Postcode {
		if id := b6.PointIDFromGBPostcode(p.Postcode[i]); id == b6.FeatureIDInvalid {
			...
			feature.ModifyOrAddTag(b6.Tag{Key: b6.PointTag, Value: b6.NewPointExpressionFromLatLng(s2.LatLngFromDegrees(p.Lat[i], p.Lng[i]))})
