...
func TestOverlayWorldReplacesPathsFromOneIndexWithAnother(t *testing.T) {
	...
	paths := b6.AllFeatures(overlay.FindFeatures(b6.Typed{b6.FeatureTypePath, b6.Tagged{Key: "#highway", Value: b6.NewStringExpression("path")}}))
	if len(paths) > 0 {
		t.Errorf("Expected to find 0 paths, found %d", len(paths))
	}

	paths = b6.AllFeatures(overlay.FindFeatures(b6.Typed{b6.FeatureTypePath, b6.Tagged{Key: "#highway", Value: b6.NewStringExpression("cycleway")}}))
	if len(paths) == 1 {
