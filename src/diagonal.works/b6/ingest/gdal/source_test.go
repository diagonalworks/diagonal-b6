...
func TestReadFeaturesFromLSOABoundaries(t *testing.T) {
	source := Source{
		...
		AddTags:    []b6.Tag{{Key: "#boundary", Value: b6.StringExpression("lsoa")}},
		IDField:    "LSOA11CD",
	...
	expected := []b6.Tag{{Key: "#boundary", Value: b6.StringExpression("lsoa")}, {Key: "name", Value: b6.StringExpression("Camden 018B")}}
...
func TestReadFeaturesFromLSOABoundariesCopyingAllFields(t *testing.T) {
	...
	expected := []b6.Tag{{Key: "code", Value: b6.StringExpression("E01000858")}, {Key: "LSOA11NM", Value: b6.StringExpression("Camden 018B")}}
	for _, tag := range expected {
		if found.Get(tag.Key) != tag {
			t.Errorf("Expected to find %s", tag)
		}
