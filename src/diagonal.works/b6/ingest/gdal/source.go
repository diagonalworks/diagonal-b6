...
func newFeatureFromS2Region(r s2.Region) ingest.Feature {
	...
	case s2.Point:
		return &ingest.GenericFeature{
			...
			Tags: []b6.Tag{{Key: b6.PointTag, Value: b6.NewPointExpressionFromLatLng(s2.LatLngFromPoint(g))}},
		}
	case *s2.Polyline:
		f := &ingest.GenericFeature{}
		for i, p := range *g {
			f.ModifyOrAddTagAt(b6.Tag{b6.PathTag, b6.NewPointExpressionFromLatLng(s2.LatLngFromPoint(p))}, i)
		}
		return f
	case *s2.Polygon:
...
func (c copyFields) Fill(f *gdal.Feature, tags []b6.Tag) ([]b6.Tag, error) {
	for _, cc := range c {
		if value, err := cc.Value(f); err == nil {
			tags = append(tags, b6.Tag{Key: cc.Key, Value: b6.NewStringExpression(value)})
		} else {
			return nil, err
		}
	}
