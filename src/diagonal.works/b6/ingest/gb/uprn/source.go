...
func (s *Source) read(r *csv.Reader, emit ingest.Emit, columns []int, goroutines int) error {
	...
	for i := range ps {
		...
		ps[i].AddTag(b6.Tag{Key: "#place", Value: b6.NewStringExpression("uprn")})
	}
	...
	for {
		...
		ps[slot].AddTag(b6.Tag{Key: "#place", Value: b6.NewStringExpression("uprn")})
		ps[slot].ModifyOrAddTag(b6.Tag{Key: b6.PointTag, Value: b6.NewPointExpressionFromLatLng(s2.LatLngFromDegrees(lat, lng))})
...
func (s *ClusterSource) Read(options ingest.ReadOptions, emit ingest.Emit, ctx context.Context) error {
	if !options.SkipPoints {
		...
		for i := range features {
			features[i] = &ingest.GenericFeature{Tags: []b6.Tag{{Key: "#place", Value: b6.NewStringExpression("uprn_cluster")}, {Key: "uprn_cluster:size", Value: b6.NewStringExpression("0")}}}
		}
		...
		for c, count := range s.centroids {
			...
			features[slot].ModifyOrAddTag(b6.Tag{Key: "uprn_cluster:size", Value: b6.NewStringExpression(strconv.Itoa(int(count)))})
			features[slot].ModifyOrAddTag(b6.Tag{Key: b6.PointTag, Value: b6.NewPointExpressionFromLatLng(c.LatLng())})
