				...
				<div className="relative flex-grow">
					<div ref={keywordsRef} className="keywords">
						{input.split(" ").map((word, index) => {
							const isFunction = functions.some((f) => f.id === word);
							const isPipe = word === "|";
							return (
								<span
									key={index}
									className={twMerge(
										"text-graphite-70",
										isFunction && "text-ultramarine-70 italic",
					...
					{functionResults.map((f) => (
						<Command.Item
							...
							onSelect={() => {
								...
								)} ${f.item.id} ${input.slice(currentWord!.pos)}`;
						...
						>
							<Line className="flex flex-row gap-1 items-center">
								...
								<div className="flex gap-2 items-baseline [&strong]:font-medium">
									<span>{highlighted(f.item.id, f.matches.id)}</span>

									<span className="text-xs text-graphite-60 ">
										{f.item?.description &&
											highlighted(f.item.description, f.matches.description)}
									</span>
								</div>
							</Line>
						</Command.Item>
