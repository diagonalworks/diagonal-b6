...
>(({ splitScreen, children, ...props }, forwardedRef) => {
	return (
		<div
			{...props}
			className={twMerge("w-full px-1 pt-2 z-10 -mb-[1px]", props.className)}
			ref={forwardedRef}
		>
			<div
				className={twMerge("grid grid-cols-1", splitScreen && "grid-cols-2")}
			>
				{children}
			</div>
		</div>
	...
	return (
		<motion.div
			...
			className={twMerge(
				...
				side === "right" && "border-x-rose-40 border-t-rose-40 bg-rose-30",
	...
	return (
		<motion.div
			...
			className={twMerge(
				...
				active &&
					(tab.side === "right" ? "border-b-rose-30" : "border-b-graphite-30"),
				tab.side === "right" ? "hover:bg-rose-30" : "hover:bg-graphite-30",
				active && (tab.side === "right" ? "bg-rose-30" : "bg-graphite-30"),
				props.className,
			)}
			transition={{
				type: "spring",
