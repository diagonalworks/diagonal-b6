				...
				{change.created && (
					<div>
						...
						>
							Scenario Details
							<ChevronDownIcon
								className={twMerge("h-4 w-4", open && "transform rotate-180")}
							/>
						</button>
					</div>
				)}
				{open && (
					<div>
						<div className="flex flex-col gap-2">
							{change?.spec.features && change.spec.features.length > 0 ? (
								change.spec.features.map((feature, i) => (
									<Line
										className={twMerge(
											"text-sm py-1 flex gap-2 items-center justify-between w-80",
											change.created && "bg-rose-10 hover:bg-rose-10 italic",
										)}
										key={i}
									>
										{feature.label ? (
											<LabelledIconAdapter labelledIcon={feature.label} />
										) : (
										...
										{!change.created && (
											<button
												...
												onClick={() => changeActions.removeFeature(id, feature)}
											>
												<Cross1Icon />
											</button>
										)}
...
const ChangeCombo = ({
	...
	const baseline = useWorldStore((state) => state.worlds?.[root ?? "baseline"]);
		...
		<div className="flex flex-col gap-2 ">
			<div>
				...
				<Combobox
					...
					onChange={(v) => {
						const option = functionResults.find((f) => f.item.label === v);
						if (!option) return;
				...
				>
					...
					<Combobox.Options className="max-h-64 overflow-y-auto border border-graphite-20 ">
						{functionResults.map((result) => (
							...
							>
								{highlighted(result.item?.label ?? "", result.matches.label)}
							</Combobox.Option>
						))}
					</Combobox.Options>
				</Combobox>
			...
			{change?.spec.changeFunction && (
				<div>
					...
					<Select.Root
						...
						onValueChange={(v) => {
							const option = analysisOptions.find(
								(analysis) => analysis.id.value?.toString() === v,
							);
							if (!option?.id) return;
					...
					>
						<Select.Trigger className=" bg-white text-graphite-70 h-10 py-2 px-2 text-sm inline-flex items-center justify-between w-full focus-within:outline-none focus-within:ring-2 focus-within:ring-rose-60/40 data-[disabled]:bg-rose-10 data-[disabled]:italic">
							<Select.Value placeholder="Select an analysis...">
								{selectedAnalysis?.label && (
									<HeaderAdapter header={selectedAnalysis.label} />
								)}
							</Select.Value>
							{!change.created && (
								<Select.Icon>
						...
						>
							<Select.Viewport>
								{analysisOptions.map((analysis: $FixMe, i: number) => {
									...
									const label = analysis.label?.title?.value;
									if (!label) return null;
									const boldWorld = label.match(/_((?:[a-zA-Z]|\d)*)_/);
									const labelNoBoldMarkers = label.replaceAll(
										boldWorld[0],
										boldWorld[1],
									);
