...
>(
	(
		{ children, className, ...props }: React.HTMLAttributes<HTMLSpanElement>,
		forwardedRef,
	) => {
		return (
			<span
				{...props}
				className={twMerge("text-graphite-100 text-base text-left", className)}
				ref={forwardedRef}
			>
				{children}
			</span>
	...
	({ children, className, ...props }: LineProps, forwardedRef) => {
		return (
			<div
				{...props}
				className={twMerge(
					"line hover:has-[.tag]:bg-white has-[.shell]:p-0 p-3 border-b flex text-left items-center gap-1 w-full min-h-11 box-border border-graphite-30 bg-white hover:bg-graphite-10 focus:bg-graphite-10 cursor-default has-[.line-button]:cursor-pointer has-[.line-button]:p-0",
					className,
				)}
				ref={forwardedRef}
			>
