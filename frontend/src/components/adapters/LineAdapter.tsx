...
export const LineAdapter = ({
	...
	const clickable = line.value?.clickExpression ?? line.action?.clickExpression;
		...
		<LineContextProvider line={line}>
			...
			>
				...
				>
								...
								{line.leftRightValue.left?.map(
									({ atom, clickExpression }, i) => {
										...
										return (
											<ClickableAtom
												atom={atom}
												clickExpression={clickExpression}
												key={i}
												key_={i}
											/>
										);
							...
							{line.leftRightValue.right?.atom && (
								<div className="flex items-center gap-1 text-ultramarine-50">
									<ClickableAtom
										...
										clickExpression={line.leftRightValue.right?.clickExpression}
									/>
								</div>
							)}
						</div>
					...
					{line.expression && (
						<span className="expression ">{line.expression.expression}</span>
					)}
					{line.error && (
					...
					{actions && (actions.show || actions.close || actions.copy) && (
				...
				return (
					...
					>
						<div className="flex gap-2 text-graphite-80 ">
							<span className=" min-w-2 italic">{tag.prefix}</span>
