...
export default function Workspace() {
	...
	const [worlds, setShellEnabled] = useWorldStore((state) => [
		state.worlds,
		state.actions.setShellEnabled,
	]);
	...
	const comparison = useMemo(() => {
		return Object.values(comparators).find(
			(comparison) =>
				...
				comparison.scenarios.some((scenario) => scenario.id === rightTab),
		);
			...
			<Tabs.Content>
				...
				<AnimatePresence>
					{splitScreen && rightTab && (
						<Tabs.Item side="right" splitScreen transition={{ duration: 0.2 }}>
							<MapProvider>
