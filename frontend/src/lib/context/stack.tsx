...
export const StackContextProvider = ({
	...
	const data = useStack(
		...
		outliner.request,
		outliner.data,
		outliner.properties.magicNumber,
	);
		...
		(node: NodeProto, shouldShowOutliner?: boolean, bustCache?: boolean) => {
			...
			actions.add({
				...
				properties: {
					...
					type: "core",
					show: true,
					showOutliner: isUndefined(shouldShowOutliner)
						? true
						: shouldShowOutliner,
	...
	// it as normal.
	if (
		!isUndefined(outliner.properties.showOutliner) &&
		!outliner.properties.showOutliner
	) {
		return <div />;
	}
